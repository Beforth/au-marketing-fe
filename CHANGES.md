...
- **Won vs Target 6-month trend (per-role, per-month targets)**:
  - Backend: `_get_hierarchical_target_for_scope` now delegates to a new `_get_hierarchical_targets_by_month` that returns a `Dict[(year, month), float]` with the exact same hierarchical logic (domain/region/employee-level monthly-target fallback chains, plus the domain/region pill shortcuts). The two pill helpers were renamed `_sum_target_for_domain`/`_sum_target_for_region` → `_targets_by_month_for_domain`/`_targets_by_month_for_region` (same fallback logic, now keyed by month).
  - `DashboardMonthPoint` gained `target_value: float = 0.0`, and the `/role-summary` 6-month trend now computes a per-month target for the trailing range and attaches it to every point (no extra endpoint).
  - Frontend: `MonthlyTrendChart` overlays a dashed amber `#f59e0b` target line (`dashArray [0,6]`, nulls where the target is 0) over the won-value area, shows a legend when 2 series are present, and was retitled "Won Value vs Target — Last 6 Months" on every dashboard. `target_value?: number` added to the `DashboardMonthPoint` type.
- **Performer of the Month is now on the Employee dashboard too**: the momentum row became `lg:grid-cols-3` with the target hero in `lg:col-span-2` + `PerformerOfMonthCard` (fetched via `getPerformerOfMonth`, non-fatal on error), matching the other three role dashboards exactly.
- **Card padding optimized**: the 5 chart cards (trend, region, status, sources, pipeline) switched content padding from `p-6` to `p-4` (`contentClassName="p-4"`, merged via tailwind-merge) — charts get more room while headers/lists keep their compact `px-5` rows.
- Files: `au-marketing-api/app/routers/dashboard.py`, `lib/marketing-api.ts`, `components/dashboard/MonthlyTrendChart.tsx`, `components/dashboard/RegionBreakdownChart.tsx`, `LeadSourceChart.tsx`, `LeadStatusChart.tsx`, `RevenuePipelineChart.tsx`, `pages/dashboards/EmployeeDashboard.tsx`

### Rev 22 — 2026-08-15 (v1.2.9) — [Revision]
- **Full frontend redesign of the live dashboard** (per request for "improved dashboard layout / card sizes" + general dashboard improvements): every role dashboard now shares one compact, data-dense skeleton.
  - **New building blocks**: `DashboardStatCard` (compact `p-4` KPI tile with a `text-[22px]` value, delta chip against the previous month, 6-point sparkline, click-through route, and an optional `right` slot — used for the won/lost donut), `KpiSparkline` (tiny inline-SVG trend under the value), `WonLostDonut` (two-segment ring showing won vs lost with a conversion % center), `TargetProgressBar` (hero with a `%` chip, progress bar, "N won this month", and days-left + projected end-of-month figure), `PerformerOfMonthCard` (compact top-3 leaderboard with trophy/rank badges).
  - **Layout per role**: 4-card KPI strip → momentum row (target hero + top-3 performer, except Employee which gets a full-width target) → full-width chart grid → full-width lists row (Recent Leads + High Value / Follow-ups Due), moved **below** the charts so the side column no longer squeezes the chart cards small. Chart cards fill their rows evenly (fixed `h-[320px]` chart area). Chart grids per role: Employee = trend + status; Region Head = full-width trend hero + status + sources; Domain Head = 2×2 trend/region/pipeline/status; Super Admin = 2×2 trend/region/sources/status.
  - **`Card` header slimmed globally** (`px-5 py-3.5 min-h-[56px]`, title `text-[15px]`) so every card in the app gets the same compact header; list rows tightened (`px-5` + smaller padding); `RecentLeadsList` gained a "View all leads →" footer.
  - **KPI cards all line up**: the won/lost donut (and any `right` slot) now renders as a right-side sibling of the info column instead of sitting in the label row — the 46px donut was pushing the Conversion card's value ~25px lower than its neighbors. Label rows reserve a fixed `min-h-5` and the subtitle/sparkline row a fixed `h-6`, so every tile has identical internal geometry: values align horizontally and cards stay equal height even when the row wraps to 2×2.
  - **Target hero no longer floats in dead space**: `TargetProgressBar` gained a bordered footer strip (Won this month · Avg/day · Projected with days left) on a `flex-1` fill row, so when it sits beside the taller Performer card (`items-stretch` momentum row) the extra height is absorbed by the stats instead of an empty bottom — the two cards now read as equal partners.
  - **Chart cards are now pixel-identical**: all 5 charts (`MonthlyTrendChart` "Won Value — Last 6 Months", `RegionBreakdownChart` "Leads by Region", `LeadStatusChart` "Leads by Status", `LeadSourceChart` "Lead Sources", `RevenuePipelineChart`) switched their chart area from a percentage-height chain (`h-full min-h-[…]`) to a fixed `h-[320px]`. ApexCharts' `height="100%"` was resolving against an indefinite flex height, so charts rendered at ragged heights inside their cards; a definite pixel height makes every chart card exactly the same size and the 2×2 grid perfectly even (`LeadStatusChart` keeps its internal scroll for many statuses).
  - **Loading state**: the center spinner was replaced with a `DashboardSkeleton` that mirrors the new layout (pulse blocks). Super Admin keeps the preview switcher, and the "Last updated" + refresh control (Rev 20) stays in the toolbar above the dashboard.
  - **A period filter (`DashboardPeriodBar`) was built, then removed in the same pass at the user's request** ("no need of time and date") — the dashboard is always current-month. The `date_from`/`date_to` plumbing added to `useRoleDashboardSummary` and the two head-summary fetches was reverted; `TargetProgressBar` no longer takes a range. The reusable `computePeriodDates` helper went away with the component.
  - **Known limitation**: `monthly_trend` is always the trailing 6 calendar months (backend-hardcoded), so delta chips are only shown on the all-time tiles (leads/orders) where "vs last month" is meaningful.
- Files: `pages/dashboards/RoleDashboardRouter.tsx`, `useRoleDashboardSummary.ts`, `EmployeeDashboard.tsx`, `RegionHeadDashboard.tsx`, `DomainHeadDashboard.tsx`, `SuperAdminDashboard.tsx`, `components/dashboard/DashboardStatCard.tsx`, `KpiSparkline.tsx`, `WonLostDonut.tsx`, `TargetProgressBar.tsx`, `PerformerOfMonthCard.tsx`, `DashboardSkeleton.tsx`, `trendUtils.ts`, `MonthlyTrendChart.tsx`, `RegionBreakdownChart.tsx`, `LeadSourceChart.tsx`, `LeadStatusChart.tsx`, `RevenuePipelineChart.tsx`, `RecentLeadsList.tsx`, `FollowUpsDueList.tsx`, `HighValueLeadsList.tsx`, `components/ui/Card.tsx`
...
- `scripts/clear_and_seed_india.py` now seeds lead / order / quotation amounts in **crore-scale rupees** (top deal ~₹9 Cr, smallest lost lead ₹80 L). Relative proportions and won/closed ratios are broadly preserved. Before this, the seeded demo never crossed the ₹1 Cr threshold where the dashboards switch to "₹X.XX Cr" formatting, so it read like a tiny business despite being named after Tata / Reliance / L&T. The quotation-attachment fallback value was bumped to match (₹1 L → ₹1 Cr). Orders and quotation attachments derive their values from the leads, so they scaled automatically. This script seeds no monthly targets, so nothing else needed rescaling.
- **Ceiling:** `Lead.potential_value` / `closed_value` are `Numeric(10, 2)` — they physically cannot hold ₹10 Cr or more. Every seeded figure is kept below ₹9 Cr; a comment in the leads table says so. (A first pass multiplied by 100 and overflowed on the ₹12 Cr / ₹11 Cr deals — hence the cap.)
...
### Rev 1 — 2026-09-03 (v1.3.1) — [Revision]
