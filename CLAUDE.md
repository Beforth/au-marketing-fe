...
### Draft leads
`Lead.is_draft` (boolean, default `False`) marks a lead saved via the create form's separate **"Save as Draft"** button (next to "Create Lead", not a checkbox) — same required fields as a normal lead (contact/customer + domain still enforced), quotation section stays visible either way, but nothing quote-related is ever sent when Save as Draft is clicked, even if the user had already filled some in. `handleSubmit` in `pages/LeadFormPage.tsx` takes a `forceDraft` argument passed directly by whichever button was clicked (not read from React state), specifically to avoid a stale-closure bug — reading `is_draft` from state set moments earlier in the same click handler would still see the pre-update value. `GET /leads/` excludes drafts by default; pass `drafts_only=true` (frontend: `getLeads({ drafts_only: true })`) to see only drafts — there's no in-between "show both" mode. Finalizing a draft is just `PUT` with `is_draft: false`; once that's done, editing it is identical to any other lead, including the quote-number flows (Inquiry #0 panel, Log Activity's "New Quotation") — those are not draft-aware, they just work because a draft is a real `Lead` row.
