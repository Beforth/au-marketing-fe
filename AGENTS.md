...
- **Two ID spaces for "who created this"**: `Domain`/`Region`/`EmployeeRegionAssignment` head/coordinator/employee fields are the HRMS `employee_id`; every `created_by_employee_id` column (Lead/Order/Contact/Customer) is actually the Django `user_id` instead (pre-existing, not a bug you introduce). Creator-chain scoping (`app/scope.py`, `get_chain_visible_creator_ids`) bridges the two via `MarketingEmployee.hrms_employee_id`/`hrms_user_id` — don't "fix" this by changing what create endpoints store; that doesn't help existing records and needs a data backfill instead.
- **`log_action(current_user=...)` param-name mismatches**: routers inconsistently name their auth param `user` vs `user_info` — always match the actual param in that specific function. `domains.py`/`series.py` both had a silent `NameError` crash (blocked every domain/series edit) from a call referencing the wrong name.
